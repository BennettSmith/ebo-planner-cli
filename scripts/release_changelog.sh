...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/release_changelog.sh 0.8.0
  ./scripts/release_changelog.sh v0.8.0
EOF
}
...
# Ensure spec.lock exists and is valid before cutting a CLI release.
...
awk -v version="${version}" -v today="${today}" -v targets_line="${targets_line}" '
  ...
  function print_version_section(   any_changes, k, i, h, n, j, has_exact_targets) {
    ...
    for (i = 1; i <= 6; i++) {
      ...
      # CLI-specific requirement: ensure release notes include "- Targets spec `vX.Y.Z`".
...
' "${CHANGELOG_FILE}" > "${tmpfile}" 2> "${tmpfile}.err" || {
  if grep -q "__EBO_NO_CHANGES__" "${tmpfile}.err"; then
    err "No changes found under ## [Unreleased]. Add entries before cutting a CLI release."
    exit 1
  fi
...
printf "OK: Cut CLI release %s (%s) in %s\n" "${version}" "${today}" "${CHANGELOG_FILE}"
